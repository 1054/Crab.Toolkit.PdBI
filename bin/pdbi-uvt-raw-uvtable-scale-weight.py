...
if len(uvt_names) == 0 or out_name == '' or np.isnan(scale_factor):
    print('Usage: ')
    print('  pdbi-uvt-raw-uvtable-scale-weight.py -name uvtable_spw1.uvt -factor 2.0 -out uvtable_spw1_scaled_weights.uvt')
    print('  -- this code will scale the weights of all visibilities in the input uv tables by the input factor.')
    sys.exit()
...
for i_uvt in range(len(uvt_names)):
    ...
    # CrabFitsHeader test_random_2_scaled_weight.uvfits > tmp_2.txt
